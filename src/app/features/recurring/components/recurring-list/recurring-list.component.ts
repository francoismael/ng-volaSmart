...
export class RecurringListComponent implements OnInit {
    ...
    selectWeekdays() {
        this.formDaysOfWeek = [1, 2, 3, 4, 5];
    }
    selectWeekend() {
        this.formDaysOfWeek = [0, 6];
    }
    selectAll() {
        this.formDaysOfWeek = [0, 1, 2, 3, 4, 5, 6];
    }
    ...
    private buildPayload(): Partial<RecurringOperation> {
        return {
            label: this.formLabel,
            amount: this.formAmount ?? 0,
            type: this.formType,
    ...
    saveForm() {
        ...
        const obs = editing ? this.recurringService.update(editing.id, payload) : this.recurringService.create(payload);
        obs.subscribe({
            next: () => {
    ...
    toggleActive(item: RecurringOperation) {
        this.recurringService.toggle(item.id, !item.isActive).subscribe({
            next: () => {
                this.toast.success(item.isActive ? 'Désactivée' : 'Activée');
                this.load();
            },
    ...
    confirmDelete(item: RecurringOperation) {
